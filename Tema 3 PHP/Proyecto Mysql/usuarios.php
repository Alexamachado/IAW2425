...
if (isset($_GET['email'])) {
    $correousuario = htmlspecialchars(trim(mysqli_real_escape_string($enlace, $_GET['email'])));
     $queryfirst = "SELECT email FROM usuarios WHERE email='$correousuario'";
     $querydelete = "DELETE FROM usuarios WHERE email='$correousuario'";
...
$todo .=      "<tr> <td>" . $fila["nombre"] . "</td>" . "<td>" . $fila["apellido"] . "</td>" . 
              ...
              "<td> " . $fila["Rol"] .  "</td> <td> <button type='button' onclick='if(confirm(\"¿Estas seguro de que deseas borrar el usuario?\")) window.location.href=\"?email=" . $fila["email"] . "\";'>Borrar</button>  </td> <td> <button type='button' onclick='window.location.href=\"?econtrasena=" . $fila["email"] . "\";'>Cambiar </button>  </td> </tr>"; 
        }
mysqli_close($enlace);
?>                          
